...
cloudinary.config(
    cloud_name="dvwg07okg",  # Replace with your Cloudinary cloud name
    api_key="482781931676338",        # Replace with your Cloudinary API key
    api_secret="grvT6Q7mEh8mWlGc-8E8G0s9S1I"   # Replace with your Cloudinary API secret
)
...
def stitch():
    ...
    if not elevation_groups:
        ...
        return jsonify({'error': 'No valid elevation groups found.'}), 400
    ...
    try:
        for elevation, group_files in elevation_groups.items():
            ...
            if pano is None:
                ...
                return jsonify({'error': f'Stitching failed for elevation {elevation}°.'}), 500
        ...
        sorted_elevations = sorted(horizontal_panoramas.keys(), reverse=True)
        ...
        for pano in resized_panoramas[1:]:
            ...
            if homography is None:
                ...
                return jsonify({'error': 'Homography could not be computed.'}), 500
        ...
        # Upload final panorama to Cloudinary
        upload_result = cloudinary.uploader.upload(BytesIO(panorama_bytes), resource_type="image")
        panorama_url = upload_result['secure_url']

        logger.info("Final panorama uploaded to Cloudinary.")

        # Send the panorama URL back
...
# @app.route('/stitch', methods=['POST'])
# def stitch():
#     print("Received a request to /stitch endpoint.")
#     logger.info("Received a request to /stitch endpoint.")
